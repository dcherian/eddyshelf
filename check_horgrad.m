...
Tx0 = diff(S.temp,1,1)./diff(xrmat,1,1);
...
h(1) = subplot(131);
contourf(avg1(squeeze(xrmat(:,1,:)),1)/1000,avg1(squeeze(zrmat(:,1,:)),1),Tgrad,20);
title('horgrad'); colorbar; cx = caxis;

h(2) = subplot(132);
contourf(avg1(squeeze(xrmat(:,1,:)),1)/1000,avg1(squeeze(zrmat(:,1,:)),1),Tx0,20);
...
h(3) = subplot(133);
contourf(avg1(squeeze(xrmat(:,1,:)),2)/1000,avg1(squeeze(zrmat(:,1,:)),2),dTz./max(dTz(:))*100);
title('% error'); colorbar;
linkaxes(h,'xy');