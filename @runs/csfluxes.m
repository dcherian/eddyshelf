...
% 2. Shelf water dye & eddy dye
    ...
    for kk=1:1 %length(indices)
        ...
        if bathyax == 2 && sgntamp == -1
            runs.csflux.off.slopewater.envelope(:,kk) = ...
                nanmax(avg1(binmat,2) .* fillnan( ...
                    double(runs.csflux.off.slopewater.trans{kk} > 0), 0), [], 2);
            runs.csflux.on.slopewater.envelope(:,kk) = ...
                nanmax(avg1(binmat,2) .* fillnan( ...
                    double(runs.csflux.on.slopewater.trans{kk} > 0), 0), [], 2);
        else
            runs.csflux.off.slopewater.envelope(:,kk) = ...
        ...
        for ll=1:kk
            ...
            runs.csflux.on.slopeztneg(:,:,kk,ll) = ...
                squeeze(nansum(bsxfun(@times, ...
                                      bsxfun(@times, squeeze(slopemask .* csvel ...
                                                             .* (csvel*sgntamp < 0) ...
                                                             .* csvonmask), ...
                                             permute(onmask, [1 3 2])), dx),1));

            runs.csflux.on.slopextneg(:,:,kk,ll) = ...
                squeeze(trapz(zvec, slopemask .* csvel ...
                              .* (csvel * sgntamp < 0) .* csvonmask,2));
