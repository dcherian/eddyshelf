...
function [] = plot_streamerstats(runs)
    ...
    line(runs.streamer.off.Vbin, repmat(avg1(bins'),[1 runs.streamer.sz4dsp(2)]));
    hold on
    zcenbin = vecfind(bins, cut_nan(runs.streamer.off.zcen));
    Vcenbin = diag(runs.streamer.off.Vbin(zcenbin,:));
    colorbar; cblabel('day');
    scatter(gca,zeros(20,1),runs.streamer.off.zcen, ...
    ...
    subplot(122); hold on
    plot(runs.streamer.time/86400,runs.streamer.off.zcen,'r');
    plot(runs.streamer.time/86400,runs.streamer.off.zdcen,'b');
