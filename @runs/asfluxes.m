...
function asfluxes(runs)
    ...
    for ii=1:length(locs)
        ...
        if do_energy
            ...
            % deep water
            runs.asflux.deep.ikeflux = squeeze(trapz(yvec(ideep), ...
                ikefluxyt(ideep,:,ii), 1));
            runs.asflux.deep.ipeflux = squeeze(trapz(yvec(ideep), ...
                ipefluxyt(ideep,:,ii), 1));
            runs.asflux.deep.eddy.ikeflux = squeeze(trapz(yvec(ideep), ...
                ikefluxyt_edd(ideep,:,ii), 1));
            runs.asflux.deep.eddy.ipeflux = squeeze(trapz(yvec(ideep), ...
                ipefluxyt_edd(ideep,:,ii), 1));

            % over shelf-slope
            runs.asflux.topo.ikeflux = squeeze(trapz(yvec(itopo), ...
                ikefluxyt(itopo,:,ii), 1));
            runs.asflux.topo.ipeflux = squeeze(trapz(yvec(itopo), ...
                ipefluxyt(itopo,:,ii), 1));
            runs.asflux.topo.eddy.ikeflux = squeeze(trapz(yvec(itopo), ...
                ikefluxyt_edd(itopo,:,ii), 1));
            runs.asflux.topo.eddy.ipeflux = squeeze(trapz(yvec(itopo), ...
