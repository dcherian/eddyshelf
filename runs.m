classdef runs < handle
    ...
    methods
        ...
        function [] = compare_plot(runs,num)
            ...
            subplot(aa,2,6); hold on
            plot(eddy.t,eddy.V,'Color',colors(ii,:));
            ylabel('velocity scale');
            liney(0); xlim(limx);
            %plot(eddy.t,eddy.cx/1000,'Color',colors(ii,:));
            %ylabel('x - center (km)');
        ...
        % 2 - use vormask
            ...
            function [] = water_census(runs)
                ...
                regsl = sparse(reshape(cs <= xsl & cs >= xsb, ...
                                       sz3dsp));
                regsh = sparse(reshape(cs < xsb, sz3dsp));

                % volume of cells
                % volume for cells in sponge region is set to 0.
                ...
                for tt=1:slab:length(runs.time)
                    ...
                    % make sure I only count if change in dye value > 1km
                    % This doesn't conserve volume in calculation, at least
                    % when I do it naively.
                    %if tt == 1
                    %    csdye0 = csdye(:,:,:,1);
                    %end
                    %dcsmask = abs(bsxfun(@minus, csdye, csdye0)) > 1000;
